...
无论您是用户还是管理员，Kibana 都通过提供三个关键功能使您的数据具有可操作性。
* 一个开源的分析和可视化平台。使用 Kibana 来探索 Elasticsearch 数据，然后构建漂亮的可视化和仪表板。
* 用于管理 Elastic Stack 的 UI。管理您的安全设置、分配用户角色、生成快照、roll up 数据等，所有这些都来自 Kibana UI 的便利性。
* 一个集中的 Elastic 解决方案集线器。从日志分析到文档发现再到 SIEM, Kibana 是访问这些功能和其他功能的门户。
